...
func get_block_by_height_url(height int64) string {
    return "https://blockchain.info/ko/block-height/" + strconv.FormatInt(height, 10) + "?format=json"
}
...
func get_block_by_height(height int64) Block {
    ...
    block.height = Get(s, "blocks.0.height").Int()
    block.block_index = Get(s, "blocks.0.block_index").Int()
    block.hash = Get(s, "blocks.0.hash").String()
    block.nonce = Get(s, "blocks.0.nonce").String()
